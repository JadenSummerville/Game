...
public class Zscore {
    ...
    Zscore(double mean, double standard_deviation, double max_value, double min_value) {
        this.mean = mean;
        this.standard_deviation = standard_deviation;
    ...
    private void checkRep() {
        if(max_value <= min_value){
            throw new RuntimeException("Rep invarient failure. Maximum value is not less than minimum value.");
        }
        if(standard_deviation <= 0){
            throw new RuntimeException("Rep invarient failure. Negative or zero standard deviation.");
        }
    }
    public double mean(){
        return mean;
    ...
    static double generate_zScore() {
        return generate_zScore(100,-100);
    }
    static double generate_zScore(double max, double min) {
    ...
    private static int loops_required(double max_value, double min_value) {
        if(max_value == min_value){
            throw new RuntimeException("The maximum value and the minimum value are the same.");
        }
        if(max_value < min_value){
            throw new RuntimeException("The maximum value and the minimum value are backwords.");
        }
